...
| 20 | `CatAvatar.tsx:263` | `overflow: 'hidden'` clips cat ears in HomeScreen score ring — left ear is a stub, right ear hidden behind progress arc | Cat ears look broken on BOTH phone and simulator (confirmed Mar 20) | **FIXED (removed overflow:hidden from hero container). Micro-animation alignment is Phase 18 Figma work.** |
...
| 60 | P1 | ElevenLabs TTS intermittent | Cat voice coaching silent on some flows | **FIXED (audio session reset before playback — was stuck in measurement mode after mic use)** |
...
| 62 | P1 | Keyboard highlights note one ahead of where playback is | Wrong note highlighted, correct input shows miss | **FIXED (unified to realtimeBeatRef, lookahead tightened 0.5→0.3)** |
...
| 65 | P2 | Replay JSON parse error | Salsa replay coaching broken | Parser works correctly, returns null on malformed JSON. Fallback replay is the real issue — deferred to Phase 13 completion (replay needs exercise type awareness). |
...
| 75 | P0 | Today's Practice disconnected from lesson tree | Exercises feel random | **FIXED (hybrid 1:1 split — lesson-tree slot + AI slot, findLessonTreeExercise reads lessonProgress)** |
...
|---|----------|-----------|-----|-------------|
| 101 | P1 | AI exercise title mismatch | Confusing — user plays one thing, results show different title | **FIXED (displayTitle from skill node, static exercises for lesson slot, Gemini title overridden)** |
| 102 | P1 | "NEW RECORD" shows on every AI exercise attempt including 16% scores — high score lookup via `ai-skill-{skillId}` key may not persist across sessions or the key doesn't match between exercise completion and lookup | False "new record" banner destroys trust in scoring | **LIKELY FIXED** (root cause: progressStore wasn't persisting due to immediateSave getter bug + `__ai__` Firestore rejection. Both now fixed. `foundPrevScore` guard already in code.) |
...
| 106 | P2 | Post-exercise Salsa coaching message appears slowly | Feels sluggish | **FIXED (cat speaks immediately using pre-computed dialogue, AI coaching loads async)** |
...
| 109 | P1 | AI exercises leak into future lessons | Lesson tree shows false progress | **FIXED (AI scores only write to started lessons, skip unstarted)** |
