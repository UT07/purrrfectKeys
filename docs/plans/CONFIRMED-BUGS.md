...
| 105 | P1 | Daily plan completion markers (green ✓) lost after sign-out/sign-in — plan itself persists (same exercises) but the `__ai__` bucket exercise scores don't survive Firestore round-trip. The `pushAllProgressData` may be silently failing for the `__ai__` document, or the pull isn't restoring it. Needs Metro log investigation: check for `[Sync] Lesson __ai__ push failed:` and `[Sync:pull] __ai__ has X scores`. ImmediateSave + Firestore undefined sanitizer added but insufficient. | Completed exercises lose their green checkmark after sign-out/sign-in |
